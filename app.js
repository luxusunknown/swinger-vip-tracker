(function () {
  ...
  function renderLatestRecap() {
    ...
    const summaryEl = document.getElementById('latestRecapSummary');
    const feedEl = document.getElementById('latestRecapFeed');
    const dateEl = document.getElementById('latestRecapDate');
    if (!section || !summaryEl || !feedEl) return;
    ...
    const daySummary = (state.dailySummaries || []).find(d => d.date === latestDate);

    const totalTrades = dayTrades.length;
    const winsCount = dayTrades.filter(t => t.win).length;
    const lossesCount = totalTrades - winsCount;
    const winRate = totalTrades ? (winsCount / totalTrades) * 100 : 0;
    const dayPnl = dayTrades.reduce((s, t) => s + (t.dollar || 0), 0);

    let potdStr = daySummary ? daySummary.playOfTheDay : null;
    let topTrade = dayTrades.slice().sort((a, b) => (b.pct || 0) - (a.pct || 0))[0];

    summaryEl.innerHTML = `
      <div class="ds-item">
        <span class="ds-label">Date</span>
        <span class="ds-val">${latestDate}</span>
      </div>
      <div class="ds-divider"></div>
      <div class="ds-item">
        <span class="ds-label">Trades</span>
        <span class="ds-val">${totalTrades} (${winsCount}W - ${lossesCount}L)</span>
      </div>
      <div class="ds-divider"></div>
      <div class="ds-item">
        <span class="ds-label">Win Rate</span>
        <span class="ds-val ${winRate >= 50 ? 'pos' : 'neg'}">${winRate.toFixed(1)}%</span>
      </div>
      <div class="ds-divider"></div>
      <div class="ds-item">
        <span class="ds-label">Total Day P&amp;L</span>
        <span class="ds-val ${dayPnl >= 0 ? 'pos' : 'neg'}">${fmtMoney(dayPnl)}</span>
      </div>
      ${potdStr ? `
      <div class="ds-divider"></div>
      <div class="ds-item">
        <span class="ds-label">Play of the Day</span>
        <span class="ds-val pos" style="font-size:13px">${potdStr}</span>
      </div>` : (topTrade ? `
      <div class="ds-divider"></div>
      <div class="ds-item">
        <span class="ds-label">Top Performer</span>
        <span class="ds-val pos" style="font-size:13px">$${topTrade.ticker} (${fmtPct(topTrade.pct)})</span>
      </div>` : '')}
    `;

    const byAnalyst = {};
    dayTrades.forEach(t => {
      (byAnalyst[t.analyst] || (byAnalyst[t.analyst] = [])).push(t);
    });

    let feedHtml = '';
    const sortedAnalysts = Object.keys(byAnalyst).sort();
    sortedAnalysts.forEach(analyst => {
      const calls = byAnalyst[analyst];
      const color = css(state.colorMap[analyst] || '--series-1');
      const analystPnl = calls.reduce((s, t) => s + (t.dollar || 0), 0);
      const analystWins = calls.filter(t => t.win).length;

      feedHtml += `
        <div class="recap-group-label">
          <span class="dot" style="background:${color}"></span>
          <span>${analyst}</span>
          <span style="font-weight:400;color:var(--text-muted)">(${analystWins}/${calls.length} wins)</span>
          <div class="recap-group-line"></div>
          <span class="recap-group-pnl ${analystPnl >= 0 ? 'pos' : 'neg'}">${fmtMoney(analystPnl)}</span>
        </div>
      `;

      calls.forEach(t => {
        const isWin = t.win;
        const isPotd = potdStr && potdStr.toUpperCase().includes(t.ticker) && potdStr.toUpperCase().includes(analyst);
        const cardClass = isWin ? 'win' : 'loss';
        const priceInfo = t.entry != null ? `@ ${t.entry.toFixed(2)}${t.exit != null ? ' → ' + t.exit.toFixed(2) : ''}` : '';

        feedHtml += `
          <div class="recap-card ${cardClass}">
            <div class="recap-card-analyst">
              <span class="dot" style="background:${color}"></span>
              ${analyst}
            </div>
            <div class="recap-card-ticker">
              $${t.ticker}
              ${isPotd ? '<span class="recap-highlight-badge">🔥 Top Play</span>' : ''}
            </div>
            ${priceInfo ? `<div class="recap-card-entry-exit">${priceInfo}</div>` : ''}
            <div class="recap-card-result">
              <span class="recap-card-badge">${isWin ? 'WIN' : 'LOSS'}</span>
              <span class="recap-card-pct">${fmtPct(t.pct)}</span>
              <span class="recap-card-dollar">${fmtMoney(t.dollar)}</span>
            </div>
          </div>
        `;
      });
    ...
    feedEl.innerHTML = feedHtml || '<div class="recap-empty">No calls recorded for this date.</div>';
  }
